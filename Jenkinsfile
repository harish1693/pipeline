      ...
      steps {
        script {
          ...
          else if (environment == 'QA')
          {
            ...
            def proc = [\'bash\', \'-c\', <ARTIFACTORY URL>/harish-DEV/\'  + " | jq -c \'.children[] | .uri\' | tr -d \'/\' " ].execute()
          ...
          else if (environment == 'UAT' || environment == 'STAGE' || environment == 'PROD')
          {
            ...
            def proc = [\'bash\', \'-c\', <ARTIFACTORY URL>/harish-Release/\'  + " | jq -c \'.children[] | .uri\' | tr -d \'/\' " ].execute()
      ...
      steps{  
        script{
                ...
                def promotionConfig = [
                      ...
                      'buildName'          : 'harish-app',
                      'buildNumber'        : buildNumber,
                      'targetRepo'         : 'harish-Release',
                   
                      // Optional parameters
                      'comment'            : 'this is the promotion comment',
                      'sourceRepo'         : 'harish-DEV',
                      'status'             : 'Released',
